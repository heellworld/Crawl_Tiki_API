...
def comment_parser(json):
    ...
    # Kiểm tra trước khi truy cập 'name'
    created_by = json.get('created_by')
    if created_by is not None:
        d['customer_name'] = created_by.get('name')
    else:
        d['customer_name'] = None  # Hoặc có thể gán một giá trị mặc định
...
df_id = pd.read_csv('product_id_smarphone.csv')
...
df_comment.to_csv('comments_data_smarphones.csv', index=False, encoding='utf-8')
